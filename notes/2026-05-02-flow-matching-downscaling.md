...
**Date**: 2026-05-02 to 2026-05-04
**Task**: 32×32 → 128×128 super-resolution of ERA5 Total Column Water (TCW)
**Metric**: CRPS (Continuous Ranked Probability Score), 10-member ensembles
**Dataset**: 40k train / 10k val / 10k test samples, values ~0.04–135 kg/m²
**Hardware**: NVIDIA L40S (48GB), MIT Engaging cluster
**Duration**: 12 iterations over ~42 hours
...
The baseline `crps_ensemble()` function uses `fc.shape[-1]**2` (=128²=16384) instead of the correct `fc.shape[0]**2` (=M²=100) as denominator in the first summation loop. This underestimates CRPS by ~50% on 128×128 data. All numbers in this report use the corrected formula: CRPS = E|X−y| − 0.5·E|X−X'| (energy form).

### GAN Mode Collapse

The baseline GAN produces near-identical predictions across different noise vectors (ensemble spread ~10⁻⁶). The adversarial loss weight (0.0001) is too small relative to MSE loss, causing the generator to ignore the 100-dim noise vector. Consequently, CRPS = MAE = 0.307 with no probabilistic benefit from ensembling.
...
```
x_0 = bicubic_upsample(LR) + σ · ε,    ε ~ N(0,I)
x_1 = HR
x_t = (1−t) · x_0 + t · x_1    (linear interpolation)
v*  = x_1 − x_0                  (target velocity)
```

The model learns a velocity field v(x_t, t, c) where c = bicubic_upsample(LR) is the conditioning input. At inference, we integrate the learned ODE from t=0 to t=1 using 20 Euler steps. Each ensemble member uses a different noise realization ε, producing natural diversity.
...
Input (B, 2, 128, 128)  [x_t concat LR condition]
  ...
  │
  ResBlock(192)
  SelfAttention(192, 4 heads)  [at 16×16 = 256 tokens]
  │
...
- 5,218,721 parameters (25× the baseline GAN generator at 204K params)
- Time conditioning via AdaGN (scale + shift from sinusoidal embedding)
- Self-attention at 16×16 bottleneck for global context
- All other layers are local 3×3 convolutions with GroupNorm
...
```
y = clamp(hr_pred, ε)
block_means = AvgPool4×4(y)
ratios = LR / block_means
hr_constrained = y · upsample_nearest(ratios, scale=4)
```

This ensures `AvgPool4×4(hr_constrained) = LR` exactly. The constraint is a simple post-hoc projection — no modification to the sampling procedure is needed.
...
| Iter | Method | Key Change | CRPS | vs Prior | vs Baseline | Reproducible |
|------|--------|-----------|------|----------|-------------|--------------|
| 1 | GAN baseline | — | 0.3066 | — | — | yes (re-run baseline) |
| 2 | Flow matching | Replace GAN with flow matching | 0.2516 | −17.9% | −17.9% | **no** (weights overwritten) |
| 3 | + mult constraint | Post-hoc conservation | 0.2460 | −2.2% | −19.8% | **no** (weights overwritten) |
| 4 | + constraint-aware | Auxiliary loss during training | 0.2424 | −1.5% | −20.9% | **no** (weights overwritten) |
| 5 | + LR-anchor | Start ODE from LR + noise | 0.2218 | −8.5% | −27.7% | **no** (weights overwritten) |
| 6 | + CRPS loss | Energy CRPS aux loss | 0.2529 | +14.0% | (failed) | **no** (weights overwritten) |
| 7 | noise_std=0.3 | Tighter noise + 200 epochs | 0.2066 | −6.9% | −32.6% | yes (`models/flow_ns03/flow_best.pth`) |
| 8 | noise_std=0.2 | Even tighter noise | 0.2065 | −0.0% | −32.6% | **no** (weights overwritten) |
| 9 | + attention | Self-attention + 48,96,192 ch | 0.2047 | −0.9% | −33.2% | **no** (checkpoint was intermediate) |
| 10 | + full training | 200 epochs (resumed from 120) | **0.1991** | **−2.7%** | **−35.1%** | **yes** (`models/flow_attn/flow_best.pth`) |
| 11 | + EMA | Exponential moving average | 0.2002 | +0.6% | (no gain) | yes (`models/flow_ema/flow_best.pth`) |
| 12 | + augmentation | H/V flips (incomplete, 85ep) | 0.2220 | — | (inconclusive) | partial (`models/flow_aug/`) |
...
| Attn + CA + none (iter 10) | 0.1995 | 0.2321 | 0.4818 | 0.2584 | 0.2108 | 0.0144 | 5.2M |
...
5. **Attention + larger model** (−0.9% CRPS): Self-attention at the 16×16 bottleneck and wider channels (48,96,192 vs 32,64,128) break through the small-model capacity ceiling.
...
1. **CRPS energy loss** (+14% CRPS): The spread reward term (−0.5|x₁−x₂|) in the differentiable CRPS loss caused the model to over-diversify at the expense of accuracy. The velocity field became less precise as the model learned to amplify noise-dependent features.
...
5. **50 Euler steps** (−0.4% CRPS at 2.5× cost): Not worth the computational overhead. With LR-anchor, the ODE trajectory is short enough that 20 steps provides adequate accuracy.
...
| ID | Decision | Outcome |
|----|----------|---------|
| DEC-001 | Use corrected CRPS formula | Baseline CRPS 0.168 → 0.307 (2× correction) |
...
| DEC-012 | 20 Euler steps sufficient | 50 steps gives only −0.4% at 2.5× cost |

## 7. Reproduction

### Valid commit for all scripts: `0433093`

All scripts needed for reproduction are at commit `0433093` (also identical at HEAD of the `research` branch). The two key files:
- `scripts/flow_downscale.py` — training and evaluation (803 lines, self-contained)
...
# Evaluate with multiplicative constraint (produces ensemble predictions)
python scripts/flow_downscale.py --mode eval \
  --data-dir external/constrained-downscaling/data/era5_sr_data \
  --save-dir models/flow_attn \
  --constraint mult --euler-steps 20 --n-members 10

# Independent CRPS verification (on saved predictions)
...
| Artifact | Path | Reproduces |
|----------|------|------------|
| Best model weights | `models/flow_attn/flow_best.pth` (20 MB) | CRPS=0.1991 via eval command above |
| Best predictions | `external/constrained-downscaling/data/era5_sr_data/prediction/flow_attn_200ep_mult_test_ensemble.pt` (6.1 GB) | CRPS=0.1991 via eval_crps.py |
| ns03 weights | `models/flow_ns03/flow_best.pth` (9.4 MB) | CRPS=0.2066 (smaller model, 32,64,128 ch) |
| EMA weights | `models/flow_ema/flow_best.pth` (20 MB) | CRPS=0.2002 |
| Augmented weights | `models/flow_aug/` (partial, 85ep) | CRPS=0.2220 (incomplete training) |

### Non-reproducible Results

Iterations 2–6 and 8 reused the same `models/flow/` directory, overwriting prior checkpoints. Their reported CRPS values (0.2516, 0.2460, 0.2424, 0.2218, 0.2529, 0.2065) are from eval logs only — the model weights no longer exist.
...
- Attention at 32×32 resolution (currently only at 16×16 bottleneck)
