"""AttentionUNet: UNet with self-attention at bottleneck for velocity/noise prediction.

Used as the backbone for both flow matching (velocity prediction) and DDPM (noise prediction).
"""
...
class ResBlock(nn.Module):
    """Residual block with time-conditioned affine modulation."""
...
class AttentionUNet(nn.Module):
    """UNet with self-attention at bottleneck.

    Input: [noisy_state, condition] concatenated along channel dim
    Output: predicted velocity (flow matching) or noise (DDPM)

    Args:
        in_channels: Total input channels (noisy + condition)
        out_channels: Output channels
        base_channels: Base feature map width
        channel_mults: Channel multipliers per resolution level
        time_emb_dim: Timestep embedding dimension
        dropout: Dropout rate
        attn_heads: Number of attention heads at bottleneck
    """
