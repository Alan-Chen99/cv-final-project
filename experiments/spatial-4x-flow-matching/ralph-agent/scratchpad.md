...
## Iteration 14
**Start:** 2026-05-08, task.resume event
**Run prefix:** (file organization — no GPU needed)

### Orientation
- Report at fixed-point (verified iter-13)
- Followup request: move ALL session-created files into `experiments/spatial-4x-flow-matching/`
- Already moved: report + task_summary (commit 6f7f36e)
- Remaining files to move (from `git diff --name-only master...HEAD`):
  - `.ralph/agent/{decisions.md, memories.md, scratchpad.md}` → `experiments/.../ralph-agent/`
  - `PROMPT.md` → `experiments/.../`
  - `logs/*.log` (9 files) → `experiments/.../logs/`
  - `scripts/{*.sbatch, cpu_eval_*.py, run_inference_ablation.sh}` (7 files) → `experiments/.../scripts/`
  - `src/exp-spatial-4x-crps-v1/*` (8 files) → `experiments/.../src/`
- Exception: CLAUDE.md — shared config. The notes table entry needs path updated to experiments location.
