#!/bin/bash
# U-ViT flow matching training: 200 epochs, then eval
# Run prefix: ecdzc-kkmks

PROJECT_DIR="/home/chenxy/repos/workspace/research3"
SIF="/home/chenxy/orcd/pool/cuda:13.0.2-cudnn-devel-ubuntu24.04.sif"
SAVE_DIR="/home/chenxy/orcd/pool/datasets/research3/models/uvit_flow_200ep"

echo "=== U-ViT Flow Training 200ep ==="
echo "Date: $(date)"
echo "Node: $(hostname)"
echo "GPU: $(nvidia-smi --query-gpu=name --format=csv,noheader 2>/dev/null || echo 'unknown')"

module load apptainer 2>/dev/null

singularity exec --nv \
    --cleanenv \
    --mount type=bind,source=/orcd,destination=/orcd \
    --mount type=bind,source=/home/chenxy/nix_store,destination=/nix,ro \
    --mount type=bind,source=${PROJECT_DIR},destination=/workspace \
    --env PREPEND_PATH=/nix/state/profile/bin:/nix/nix_path/bin \
    --env NIX_REMOTE=daemon \
    ${SIF} \
    bash -c "cd /workspace && source .venv/bin/activate && \
python src/exp-spatial-4x-crps-v1/uvit_flow.py \
    --mode train \
    --epochs 200 \
    --batch_size 64 \
    --lr 1e-4 \
    --hidden_size 256 \
    --depth 12 \
    --num_heads 4 \
    --save_dir ${SAVE_DIR} && \
echo 'TRAINING COMPLETE' && \
python src/exp-spatial-4x-crps-v1/uvit_flow.py \
    --mode eval \
    --n_ensemble 10 \
    --split test \
    --ode_steps 10 \
    --constraint addcl \
    --save_dir ${SAVE_DIR} && \
echo 'FULL EVAL COMPLETE'"

echo ""
echo "=== Done ==="
echo "Date: $(date)"
