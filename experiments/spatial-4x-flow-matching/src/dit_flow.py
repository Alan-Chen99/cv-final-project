"""
DiT Flow Matching: Diffusion Transformer for 32x32 -> 128x128 downscaling.

Replaces UNet backbone with a pure Transformer (DiT) architecture.
Uses the same OT-CFM training and residual prediction as flow_matching_v2.py.

Architecture:
  - Patch embedding (patch_size=8, 2ch -> d_model)
  - N transformer blocks with AdaLN-Zero conditioning
  - Unpatch to full resolution
  - ~12-14M parameters at d_model=384, depth=12

Usage:
  python src/exp-spatial-4x-crps-v1/dit_flow.py --mode train --epochs 40 --batch_size 64
  python src/exp-spatial-4x-crps-v1/dit_flow.py --mode eval --n_ensemble 10
"""
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=128, patch_size=8, in_channels=2, embed_dim=384):
        super().__init__()
...
class DiTBlock(nn.Module):
    ...
    def __init__(self, hidden_size, num_heads, mlp_ratio=4.0):
        ...
        # AdaLN-Zero: 6 parameters per block (shift1, scale1, gate1, shift2, scale2, gate2)
    ...
    def forward(self, x, c):
        # c: (B, hidden_size) - conditioning (time embedding)
...
class DiT(nn.Module):
    ...
    Input: noisy state (1ch) + LR condition (1ch) = 2ch at 128x128
    Output: predicted velocity (1ch) at 128x128
    """
...
def train(args):
    ...
    print(f"DiT: patch_size={args.patch_size}, hidden={args.hidden_size}, "
...
def evaluate(args):
    ...
    model = DiT(
        ...
        in_channels=2,
        out_channels=1,
        hidden_size=saved_args.get('hidden_size', 384),
    ...
    print(f"Model: DiT, patch_size={saved_args.get('patch_size', 8)}, "
          f"hidden={saved_args.get('hidden_size', 384)}, depth={saved_args.get('depth', 12)}")
